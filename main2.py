...
from PIL import ImageFile, Image, ImageDraw, PngImagePlugin, ImageChops
...
def parse():
    ...
    vq_parser.add_argument("-ckpt", "--vqgan_checkpoint", type=str, help="VQGAN checkpoint", default=f'checkpoints/2Flast.ckpt',
                           dest='vqgan_checkpoint')
    vq_parser.add_argument("-conf", "--vqgan_config", type=str, help="VQGAN config", default=f'checkpoints/2Flast.yaml', dest='vqgan_config')
    ...
    vq_parser.add_argument("-i",    "--iterations", type=int, help="Number of iterations", default=1000, dest='max_iterations')
    ...
    vq_parser.add_argument("-o",    "--output", type=str, help="Output filename", default="output", dest='output')
...
def checkin(i, losses):
    ...
    filenamestring = args.output + "_" + str(i) + ".png"
    TF.to_pil_image(out[0].cpu()).save(filenamestring, pnginfo=info)
    watermark(filenamestring)
